...
public class User extends Thread{
    ...
    public void playPinnacleStrategy(){
        int count = 0;
        for(int match_id = 1; match_id <= Database.getMatchsSize(); match_id++){
            List<BetDAO> betDAOS = Database.getBetsByMatchId(match_id);
            for(BetDAO betDAO:betDAOS){
                if(betDAO.getBookmaker().getBookmakerName().equals("Pinnacle")){
                    double sum = 1/betDAO.getFirst() + 1/betDAO.getDraw() + 1/betDAO.getSecond();
                    double newFirst = betDAO.getFirst()*sum;
                    double newDraw = betDAO.getDraw()*sum;
                    double newSecond = betDAO.getSecond()*sum;
                    System.out.printf("%f, %f, %f\n", newFirst, newDraw, newSecond);
                    for(BetDAO betDAO_:betDAOS){
                        if(betDAO_.getBookmaker().getBookmakerName().equals("Pinnacle")){continue;}
                        List<HighDAO> highDAOS = new ArrayList<>();
                        if(betDAO_.getFirst() > newFirst){
                            HighDAO highDAO = new HighDAO(String.valueOf(betDAO_.getFirst()), WinnerTypes.FIRST, HighTypes.BET,null, betDAO_.getMatchDAO(),betDAO_.getBookmaker());
                            highDAOS.add(highDAO);
                        }
                        if(betDAO_.getDraw() > newDraw){
                            HighDAO highDAO = new HighDAO(String.valueOf(betDAO_.getDraw()), WinnerTypes.DRAW, HighTypes.BET,null, betDAO_.getMatchDAO(),betDAO_.getBookmaker());
                            highDAOS.add(highDAO);
                        }
                        if(betDAO_.getSecond() > newSecond){
                            HighDAO highDAO = new HighDAO(String.valueOf(betDAO_.getSecond()), WinnerTypes.SECOND, HighTypes.BET,null, betDAO_.getMatchDAO(),betDAO_.getBookmaker());
                            highDAOS.add(highDAO);
                        }
                        count += highDAOS.size();
                        highDAOS.forEach(highDAO -> {
                            System.out.println(highDAO);
